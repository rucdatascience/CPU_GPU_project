    ...
    while (std::getline(base_line, line)) {
        ...
        if (id_res[v_id] != std::stol(tokens[1])) {
            if(!(id_res[v_id] == INT_MAX && std::stol(tokens[1]) == LLONG_MAX)){
                ...
                std::cout << "Baseline file: " << tokens[0] << " " << tokens[1] << std::endl;
                std::cout << "CPU BFS result: " << graph.vertex_id_to_str[v_id] << " " << id_res[v_id] << std::endl;
                base_line.close();
                return;
            }
        }
...
void SSSP_checker(graph_structure<double>& graph, std::vector<std::pair<std::string, double>>& res, bool& is_pass) {
    ...
    while (std::getline(base_line, line)) {
        ...
        if (fabs(id_res[v_id] - std::stod(tokens[1])) > 1e-4) {
            std::cout << "Baseline file and CPU SSSP results are not the same!" << std::endl;
            std::cout << "Baseline file: " << tokens[0] << " " << tokens[1] << std::endl;
            std::cout << "CPU SSSP result: " << graph.vertex_id_to_str[v_id] << " " << id_res[v_id] << std::endl;
            base_line.close();
            return;
        }
        id++;
...
void PR_checker(graph_structure<double>& graph, std::vector<std::pair<std::string, double>>& res, bool& is_pass) {
    ...
    while (std::getline(base_line, line)) {
        ...
        if (fabs(id_res[v_id] - std::stod(tokens[1])) > 1e-4) {
            std::cout << "Baseline file and CPU PageRank results are not the same!" << std::endl;
            std::cout << "Baseline file: " << tokens[0] << " " << tokens[1] << std::endl;
            std::cout << "CPU PageRank result: " << graph.vertex_id_to_str[v_id] << " " << id_res[v_id] << std::endl;
            base_line.close();
            return;
        }
        id++;
