...
std::vector<double> csr_sssp(KEY_TYPE *keys, VALUE_TYPE *values, SIZE_TYPE *row_offsets,
                             SIZE_TYPE node_size, SIZE_TYPE edge_size, SIZE_TYPE start_node, double INF = 10000000000);
std::vector<std::pair<std::string, double>> Cuda_SSSP(graph_structure<double> &graph, GPMA& gpma, std::string src_v, double max_dis = 10000000000);
...
__global__ void csr_sssp_gather_kernel(SIZE_TYPE *node_queue, SIZE_TYPE *node_queue_offset,
...
template<SIZE_TYPE THREADS_NUM>
__global__ void csr_sssp_relax_kernel(KEY_TYPE *edge_queue, SIZE_TYPE *edge_queue_offset,
        SIZE_TYPE *node_queue, SIZE_TYPE *node_queue_offset, double *distances, int *keys, double *values, int *bitmap) {
    typedef cub::BlockScan<SIZE_TYPE, THREADS_NUM> BlockScan;
    ...
    while (cta_offset < edge_queue_offset[0]) {
        ...
        if (edge_idx < edge_queue_offset[0]) {
            ...
            neighbour = v;
            _atomicMinDouble(&distances[v], distances[u] + values[edge_idx]);
...
__host__ std::vector<double> csr_sssp(int *keys, double *values, int *row_offsets,
    ...
    while (true) {
        ...
        cudaDeviceSynchronize();
        csr_sssp_gather_kernel<THREADS_NUM><<<BLOCKS_NUM, THREADS_NUM>>>(node_queue, node_queue_offset, 
        ...
        cudaDeviceSynchronize();
        csr_sssp_relax_kernel<THREADS_NUM><<<BLOCKS_NUM, THREADS_NUM>>>(edge_queue, edge_queue_offset, 
            node_queue, node_queue_offset, distances, keys, values, bitmap);
        cudaDeviceSynchronize();
...
std::vector<std::pair<std::string, double>> Cuda_SSSP(graph_structure<double> &graph, CSR_graph<double> &csr_graph, std::string src_v, double max_dis) {
    ...
    std::vector<double> ssspVecGPU = csr_sssp(csr_graph.out_edge, csr_graph.out_edge_weight, csr_graph.out_pointer, V, E, src_v_id, max_dis);
