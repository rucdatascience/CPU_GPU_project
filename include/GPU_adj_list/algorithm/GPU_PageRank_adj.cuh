...
__global__ void importance(double *npr, double *pr,  double damp, cuda_vector<std::pair<int, double>> **in_edge, int GRAPHSIZE);
__global__ void calculate_sink(double *pr, int *N_out_zero_gpu, int out_zero_size, double *sink_sum);
__global__ void initialization(double *pr, double *outs, cuda_vector<std::pair<int, double>> **out_edge, int N);
...
__global__ void importance(double *npr, double *pr,  double damp, cuda_vector<std::pair<int, double>> **in_edge, int GRAPHSIZE) {
    ...
    if (tid >= 0 && tid < GRAPHSIZE) {
        ...
        for (int c = 0; c < in_edge_size; c++)
            acc += pr[(*in_edge[tid])[c].first]; // val_col[c] is neighbor,rank get PR(u) row_value is denominator i.e. Nout
