...
__global__ void Relax(cuda_vector<std::pair<int, double>>** out_edge, double* dis, int* queue, int* queue_size, int* visited) {
    ...
    if (idx < *queue_size) {
        ...
        for (int i = 0; i < out_edge_size; i++) {
            int new_v = (*out_edge[v])[i].first;
            double weight = (*out_edge[v])[i].second;
...
void gpu_shortest_paths(GPU_adj<double>& input_graph, int source, std::vector<double>& distance, double max_dis) {
    ...
    *next_queue_size = 0;
    ...
    while (*queue_size > 0) {
		...
		Relax <<< numBlocks, threadsPerBlock >>> (out_edge, dis, queue, queue_size, visited);
		cudaDeviceSynchronize();
